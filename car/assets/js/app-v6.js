...
(function () {
  ...
  function openSection(catId) {
    ...
    if (prevId || nextId) {
      ...
      if (prevId) {
        const prevCat = CATEGORIES.find(c => c.id === prevId);
        html += `<button class="section-nav-btn prev" data-next="${prevId}">${prevCat.icon} ${prevCat.title}</button>`;
      }
      if (nextId) {
        const nextCat = CATEGORIES.find(c => c.id === nextId);
        html += `<button class="section-nav-btn next" data-next="${nextId}">${nextCat.title} ${nextCat.icon}</button>`;
      }
      html += `</div>`;
    }
